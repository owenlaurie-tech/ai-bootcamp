...
-- ============================================
-- Table 5: task_history (Optional - for audit trail)
-- Tracks task changes for undo/history features
-- ============================================
...
-- Users can manage tags for their own tasks
...
-- ============================================
-- Recurring Patterns Table Policies
-- ============================================

-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Users can manage own recurring patterns" ON recurring_patterns;

-- Users can manage recurring patterns for their own tasks
...
-- Enable realtime for tasks table (only if not already added)
DO $$
BEGIN
    -- Check and add tasks table
    IF NOT EXISTS (
    ...
    -- Check and add recurring_patterns table
    IF NOT EXISTS (
        SELECT 1 FROM pg_publication_tables
        WHERE pubname = 'supabase_realtime'
        AND schemaname = 'public'
...
-- Migration Complete
-- ============================================
-- This script creates all necessary tables, indexes, functions, triggers, and RLS policies
-- for the Apple-Style To-Do App with Supabase backend.
--
...
-- 1. Update supabase-config.js with your actual Supabase anon key
-- 2. Test user signup and task creation
